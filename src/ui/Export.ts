...
export function generateSTL(
	...
	for (let i = 0; i < filaments.length; i++) {
		...
		if (i == 0) {
			...
			swapString += `- Swap to ${filament.name} at a layer height of ${Math.round((filaments[i - 1].endHeight + projectConfig.layerHeight) * 100) / 100
				} mm\n`;
		}
	}

	let instructionString = `Generated by Filament Painter.
